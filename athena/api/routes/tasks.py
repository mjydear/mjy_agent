...
def _build_factory(
    service: AthenaWebService, request: TaskSubmitRequest
) -> Any:
    ...
    if request.kind == "chat":
        ...
        async def chat_factory() -> dict[str, Any]:
            resp = await service.chat(session_id, message)
    ...
    if request.kind == "workflow":
        ...
        async def workflow_factory() -> dict[str, Any]:
            resp = await service.run_workflow(task, workflow_type)
            return resp.model_dump()
...
async def submit_task(
    ...
    factory = _build_factory(service, payload)
