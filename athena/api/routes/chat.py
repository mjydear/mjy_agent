...
async def chat(
    ...
    config = service.resolve_llm_config(
        tenant, request.llm_config_id, request.llm_config
    )
...
async def stream_chat(
    ...
    # Validate before opening the SSE response so a cross-tenant id returns a
    # normal 404 JSON error instead of failing after response headers are sent.
    service.get_session(request.session_id, tenant=tenant)
