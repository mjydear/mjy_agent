...
) -> tuple[str, tuple[ReplayCase, ...]]:
    version = getattr(runner, "runner_version", default_version)
    cases_method = getattr(runner, "replay_cases", None)
    cases = cases_method() if callable(cases_method) else default_cases
    return str(version), tuple(cases)


def _is_ecommerce_candidate(candidate: object) -> bool:
    environment = str(getattr(candidate, "environment_type", "")).strip().casefold()
    workflow = str(getattr(candidate, "workflow_type", "")).strip().casefold()
    allowed_tools = set(getattr(candidate, "allowed_tools", ()))
    return (
        environment == "ecommerce"
        or workflow == "ecommerce_diagnosis"
        or bool(allowed_tools & ECOMMERCE_REPLAY_TOOL_NAMES)
    )
...
class SkillEvaluationService:
    def __init__(
        ...
        shadow_runner: SkillShadowRunner | None = None,
        ecommerce_replay_ab_runner: SkillReplayABRunner | None = None,
        ecommerce_shadow_runner: SkillShadowRunner | None = None,
    ) -> None:
        self._repository = repository
        self._runner = runner or SkillBaselineRunner()
        self._candidate_repository = candidate_repository
        self._replay_ab_runner = replay_ab_runner or SkillReplayABRunner()
        self._shadow_runner = shadow_runner or SkillShadowRunner()
        self._ecommerce_replay_ab_runner = (
            ecommerce_replay_ab_runner or EcommerceSkillReplayABRunner()
        )
        self._ecommerce_shadow_runner = (
            ecommerce_shadow_runner or EcommerceSkillShadowRunner()
        )
    ...
    async def run_replay_ab(self, tenant_id: str, candidate_id: str) -> ReplayABRun:
        ...
        replay_runner = (
            self._ecommerce_replay_ab_runner
            if _is_ecommerce_candidate(candidate)
            else self._replay_ab_runner
        )
        runner_version, cases = _runner_metadata(
            replay_runner,
            default_version=REPLAY_AB_RUNNER_VERSION,
            default_cases=fixed_replay_cases(),
        )
    ...
    async def run_shadow(self, tenant_id: str, candidate_id: str) -> ShadowRun:
        if self._candidate_repository is None:
            raise SkillCandidateLifecycleError("SKILL_CANDIDATE_SHADOW_UNAVAILABLE")
        candidate = await self._candidate_repository.get(tenant_id, candidate_id)
        if candidate is None:
            raise SkillCandidateLifecycleError("SKILL_CANDIDATE_NOT_FOUND")
        validation = await self._candidate_repository.latest_validation_for_candidate(
        ...
        shadow_runner = (
            self._ecommerce_shadow_runner
            if _is_ecommerce_candidate(candidate)
            else self._shadow_runner
        )
        runner_version, cases = _runner_metadata(
            shadow_runner,
            default_version=SHADOW_RUNNER_VERSION,
            default_cases=shadow_replay_cases(),
        )
        case_digest = replay_case_definition_digest(cases)
        existing = await self._repository.find_shadow(
            tenant_id,
            candidate_id,
            validation.candidate_digest,
            case_digest,
        ...
        try:
            ...
            run = ShadowRun(
                ...
                comparisons=(),
                aggregate={},
                gate_checks={"runtime_observed": False},
                gate_passed=False,
                failure_reason="SKILL_SHADOW_EXECUTION_FAILED",
                started_at=started_at,
                completed_at=completed_at,
            )
        return await self._repository.save_shadow(run)

    async def shadow(self, tenant_id: str, run_id: str) -> ShadowRun | None:
        return await self._repository.get_shadow(tenant_id, run_id)
