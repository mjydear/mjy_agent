...
from athena.bootstrap import build_agent as _bootstrap_build_agent
...
build_agent = _bootstrap_build_agent
...
def worker(
    ...
    once: bool = typer.Option(
        False, "--once", help="Process one worker iteration then exit."
    ),
) -> None:
    """Run the durable Outbox relay and Redis Streams Agent worker."""
    try:
        from athena.application.worker_runtime import run_worker

        asyncio.run(run_worker(load_settings(config), once=once))
    except (AthenaError, RuntimeError, ImportError) as exc:
        typer.secho(str(exc), fg=typer.colors.RED)
        raise typer.Exit(code=1) from exc
