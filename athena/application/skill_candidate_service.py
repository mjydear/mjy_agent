...
class SkillCandidateService:
    ...
    def __init__(
        ...
        *,
        allowed_readonly_tool_names: Iterable[str] = (),
    ) -> None:
        self._repository = repository
        self._source_resolver = source_resolver
        self._allowed_readonly_tools = {
            item.name for item in ReadOnlyToolCatalog().declarations if item.readonly
        }
        self._allowed_readonly_tools.update(
            name.strip() for name in allowed_readonly_tool_names if name.strip()
        )
        if validator is None:
            tool_specs = {
                item.name: item.as_spec() for item in ReadOnlyToolCatalog().declarations
            }
            for name in self._allowed_readonly_tools:
                if name not in tool_specs:
                    tool_specs[name] = ToolDeclaration(
                        name=name,
                        description="An explicitly registered read-only domain tool.",
                        input_schema={
                            "type": "object",
                            "additionalProperties": True,
                        },
                    ).as_spec()
            validator = SkillCandidateValidator(tool_specs=tool_specs)
    ...
    async def propose_from_trajectories(
        ...
        for trajectory_id in proposal.source_trajectory_ids:
            ...
            if trajectory is None:
                raise SkillCandidateSourceError("SKILL_CANDIDATE_TRAJECTORY_NOT_FOUND")
            if (
        ...
        description = normalize_safe_summary(proposal.description, field="description")
        ...
        source_summary: dict[str, object] = {
            ...
            "quality_scores": [item.admission.quality_score for item in trajectories],
    ...
    async def list_candidates(
        ...
        return await self._repository.list_deduplication_candidates(
    ...
    async def get_candidate(
        ...
        return await self._repository.get(tenant_id, candidate_id)
    ...
    def _validate_proposal(proposal: SkillCandidateProposal) -> None:
        ...
        if not proposal.capabilities:
            raise SkillCandidateSourceError(
                "SKILL_CANDIDATE_READONLY_CAPABILITY_REQUIRED"
            )
        if any(
    ...
    def _validate_trajectory_proposal(
        ...
        if not proposal.source_trajectory_ids:
            raise SkillCandidateSourceError("SKILL_CANDIDATE_TRAJECTORY_REQUIRED")
        if len(set(proposal.source_trajectory_ids)) != len(
            proposal.source_trajectory_ids
        ):
            raise SkillCandidateSourceError("SKILL_CANDIDATE_DUPLICATE_TRAJECTORY")
        ...
        if not proposal.failure_recovery or not proposal.success_contract:
            raise SkillCandidateSourceError("SKILL_CANDIDATE_CONTRACT_REQUIRED")
        ...
        if not 1 <= proposal.token_budget_hint <= 120_000:
            raise SkillCandidateSourceError("SKILL_CANDIDATE_TOKEN_BUDGET_INVALID")
    ...
    def _validate_source(
        ...
        if (
            source.outcome_id != proposal.outcome_id
            or source.feedback_id != proposal.feedback_id
        ):
    ...
    def _normalize_safe_structure(value: object, field: str) -> object:
        ...
        if isinstance(value, dict):
            return {
                str(key): SkillCandidateService._normalize_safe_structure(item, field)
