...
class ReActAgent:
    ...
    trace_recorder: StructuredTraceRecorder | None = None
    last_trace_run_id: str | None = field(default=None, init=False)
    ...
    def _trace(self, method: str, **payload: object) -> None:
        """Emit optional redacted telemetry without affecting agent execution."""
        if self.trace_recorder is None or self.last_trace_run_id is None:
            return
        try:
            getattr(self.trace_recorder, method)(self.last_trace_run_id, **payload)
        except Exception:  # noqa: BLE001 - observability is a side channel
            logger.warning("legacy trace recorder failed: event=%s", method)
    ...
    async def run(self, query: str) -> AgentResponse:
        ...
        if self.trace_recorder is not None:
            try:
                self.last_trace_run_id = self.trace_recorder.start_run()
            except Exception:  # noqa: BLE001 - observability is a side channel
        ...
        try:
            ...
            for step_index in range(1, self.max_steps + 1):
                ...
                if decision.final_answer and decision.action is None:
                    ...
                    self._trace("finish_run", status="succeeded")
                ...
                # --- 步骤 6：执行工具调用 ---
                self._trace(
                    "record_tool_started", step=step_index, action=decision.action
                )
                ...
                self._trace(
                    ...
                    error_code=None if tool_result.success else "TOOL_EXECUTION_FAILED",
                )
                # 💡 学习提示：工具可能成功也可能失败。
            ...
            self._trace(
                "finish_run",
                status="failed",
                error_code="AGENT_STEP_BUDGET_EXHAUSTED",
            )
            ...
            self._trace(
                "finish_run", status="failed", error_code="AGENT_EXECUTION_FAILED"
            )
            raise
        except AthenaError:
            # ② AthenaError：其他 Athena 层异常（如 LLMError、ToolError），也直接抛
            # 上层调用者（CLI）会统一处理这些有 error_code 的异常，显示友好错误信息
            self._trace("finish_run", status="failed", error_code="ATHENA_ERROR")
            raise
