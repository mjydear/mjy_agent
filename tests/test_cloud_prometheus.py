...
def test_prometheus_mock_queries_common_metrics() -> None:
    ...
    assert {metric.data_origin for metric in pod_metrics} == {"mock"}
...
def test_prometheus_real_http_failure_is_unavailable() -> None:
    ...
    assert result.error_code == ErrorCode.ENV_METRICS_UNAVAILABLE
    assert result.retryable is True


def test_prometheus_fail_closed_rejects_mock_endpoint() -> None:
    client = PrometheusQueryClient(
        enabled=True,
        base_url="mock://prometheus",
    ...
    assert result.error_code == ErrorCode.ENV_DATA_ORIGIN_FORBIDDEN


def test_prometheus_permission_failure_has_stable_error_code(
    monkeypatch,
) -> None:
    ...
    def reject(*args: object, **kwargs: object) -> object:
        raise ForbiddenError("sensitive provider detail")

    monkeypatch.setattr(client, "_real_query", reject)
    result = client.query("up", "up")
