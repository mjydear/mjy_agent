...
🔗 依赖关系：
   - 依赖：
     ...
     * athena/prompt/templates/base.md → 系统级提示（system_prompt 槽位）
...
class ContextAssembler(BaseModel):
    ...
    字段说明（4 个配置项）：
        ...
        system_prompt_path:
            ...
            文件不存在时自动降级为空字符串（不报错）。
            默认值：athena/prompt/templates/base.md
    ...
    使用示例：
        ...
        # 自定义系统提示词路径和补充上下文
        assembler = ContextAssembler(
            system_prompt_path=Path("athena/prompt/templates/custom.md"),
    ...
    system_prompt_path: Path = _TEMPLATE_DIR / "base.md"
    template_path: Path = _TEMPLATE_DIR / "react.md"
    static_context: str = ""
    ...
    def _read_optional(self, path: Path) -> str:
        ...
        使用示例：
            content = self._read_optional(_TEMPLATE_DIR / "base.md")
