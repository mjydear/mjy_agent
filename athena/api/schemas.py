...
class SessionSummary(BaseModel):
    ...
    message_count: int
    preview: str = (
        ""  # 最后一条消息的单行预览文本，供侧边栏两行会话项展示；无消息时为空串。
    )
...
class LLMConfig(BaseModel):
    """Per-request LLM credentials supplied by the Web Console."""

    provider: str = "litellm"
    model: str = Field(default="deepseek/deepseek-chat", min_length=1, max_length=120)
...
class LLMConfigRotate(BaseModel):
    """A replacement credential that is never serialized back to the caller."""

    api_key: SecretStr


class LLMConfigTestResult(LLMConfigPublic):
    success: bool
    reason_code: str
...
class ChatRequest(BaseModel):
    ...
    llm_config_id: str | None = Field(default=None, max_length=80)
