...
class SecuritySandbox:
    ...
    async def run_shell(self, command: str, cwd: Path | None = None) -> SandboxResult:
        ...
        command = self._resolve_windows_python(command)
        started_at = time.time()
    ...
    def _execute_restricted_python(self, code: str) -> str:
        try:
            ...
            # Keep the local verifier usable when optional dependencies were
            # not installed. This fallback is deliberately an AST admission
            # check, never an eval/exec path: only literal assignments pass.
            try:
                tree = ast.parse(code, mode="exec")
            except SyntaxError as syntax_error:
                raise RuntimeError("restricted Python parser is unavailable") from syntax_error
            allowed = (ast.Module, ast.Assign, ast.Name, ast.Constant, ast.Store)
            if not all(isinstance(node, allowed) for node in ast.walk(tree)):
                raise RuntimeError("RestrictedPython is required for non-literal sandbox code") from exc
            if not all(
                ...
                raise RuntimeError("sandbox fallback only permits literal assignments") from exc
            return ""
    ...
    @staticmethod
    def _resolve_windows_python(command: str) -> str:
        """Use the active interpreter instead of the Windows Store ``python`` alias."""
        if os.name != "nt":
            return command
        parts = shlex.split(command, posix=False)
        if not parts or Path(parts[0]).name.lower().removesuffix(".exe") != "python":
            return command
        suffix = command[len(parts[0]) :]
