...
from athena.api.session_store import tenant_id_for
...
from athena.exceptions import AthenaError, ErrorCode, OpsError
from athena.integration.alert_webhook import AlertWebhookPayload
...
from athena.observability.decision_trace import StructuredTraceRecorder, TaskTraceEvent
...
if TYPE_CHECKING:
    ...
    from athena.infra.secret_store import SecretStore
...
class AthenaWebService:
    ...
    def __init__(
        ...
        llm_config_store: object | None = None,
        allow_legacy_llm_credentials: bool = True,
        ...
        ops_security: "OpsSecuritySettings | None" = None,
    ) -> None:
        ...
        self.decision_trace = StructuredTraceRecorder(self.tracer)
        self._task_decision_traces: dict[str, tuple[TaskTraceEvent, ...]] = {}
        ...
        self.llm_config_store = llm_config_store or LLMConfigStore(
            getattr(self.session_store, "_cache"), secret_store
        )
        self._allow_legacy_llm_credentials = allow_legacy_llm_credentials
        ...
        self._agent_config_signatures: dict[
        ...
        self._ops_security = ops_security
        self._alert_history: list[dict[str, object]] = []

    def configure_ops(
        self,
        diagnoser: "K8sReadOnlyDiagnoser",
        security: "OpsSecuritySettings",
    ) -> None:
        """Bind application-scoped CloudOps dependencies when none were injected."""
        if self._k8s_diagnoser is None:
            self._k8s_diagnoser = diagnoser
        if self._ops_security is None:
            self._ops_security = security
    ...
    def list_sessions(
        ...
        """
        self.cleanup_expired_sessions(tenant=tenant)
        return [
            self._summary_from_stored(stored)
            for stored in self.session_store.list(tenant=tenant)
        ]
    ...
    def get_session(
        ...
        """
        self.cleanup_expired_sessions(tenant=tenant)
        return self._session_detail(self._require_session(session_id, tenant=tenant))
    ...
    def delete_session(
        ...
        self.cleanup_expired_sessions(tenant=tenant)
        if self.session_store.get(session_id, tenant=tenant) is None:
            raise ApiServiceError(
                "SESSION_NOT_FOUND", f"Session not found: {session_id}", status_code=404
            )
        self.session_store.delete(session_id, tenant=tenant)
        runtime_key = self._runtime_key(tenant, session_id)
        self._agents.pop(runtime_key, None)
        self._agent_config_signatures.pop(runtime_key, None)
    ...
    async def chat(
        ...
        *,
        ...
        session = self._require_session(session_id, tenant=tenant)
        agent = self._ensure_agent(session, llm_config, tenant=tenant)
        ...
        record = TaskRecord(task_id=task_id, status="running", tenant_id=tenant_id)
        self.task_store.save(record, tenant=tenant)
        try:
            ...
            self.task_store.save(record, tenant=tenant)  # 任务结果落库，重启/多副本可查
            self._project_legacy_trace(agent, task_id, tenant_id=tenant_id)
            return ChatResponse(
                task_id=task_id,
            ...
            self._record_failure(record, exc, started_at, tenant=tenant)
            self._project_legacy_trace(agent, task_id, tenant_id=tenant_id)
            self._audit(tenant_id, "chat.run", task_id, False, exc.message)
            raise ApiServiceError(str(exc.code), exc.message) from exc
        except Exception as exc:
            self._record_failure(
                record, exc, started_at, tenant=tenant
    ...
    async def run_workflow(
        ...
        *,
        ...
        tenant_id = tenant_id_for(tenant) if tenant is not None else (actor or "public")
        ...
        try:
            ...
            record.updated_at = time.time()
            self.task_store.save(record, tenant=tenant)  # 任务结果落库，重启/多副本可查
            self._audit(tenant_id, "workflow.run", task_id, True, workflow_type)
            return WorkflowRunResponse(
                task_id=task_id, status="success", answer=response.answer, steps=steps
            )
        except Exception as exc:
            self._record_failure(record, exc, started_at, tenant=tenant)
            self._audit(tenant_id, "workflow.run", task_id, False, str(exc))
    ...
    def get_task_status(
        ...
        """
        record = self._require_task(task_id, tenant=tenant)
    ...
    def get_traces(self, task_id: str, *, tenant: "TenantContext") -> TraceResponse:
        """
        查询任务执行轨迹。
        ...
        """
        record = self.task_store.get(task_id, tenant=tenant)
        if record is None or record.tenant_id != tenant_id_for(tenant):
            # Missing and cross-tenant traces deliberately share the same response
            # so task ownership cannot be inferred from a guessed identifier.
            raise ApiServiceError("TRACE_NOT_FOUND", "Trace not found", status_code=404)
    ...
    def list_audit_events(
        ...
        *,
        ...
        """Return audit events scoped to the authenticated tenant.

        ``target_tenant_id`` is intentionally reserved for an explicitly scoped
        system auditor.  Legacy in-process callers without a tenant keep their
        diagnostic all-tenant view; HTTP routes always supply ``tenant``.
        """
        ...
        if tenant is None:
            if target_tenant_id is not None:
                raise ApiServiceError(
                    "FORBIDDEN",
                    "tenant override requires an authenticated auditor",
                    403,
                )
            return [_asdict(record) for record in store.list_all(limit=limit)]
        if target_tenant_id is not None and not tenant.has_scope("audit:read:any"):
            raise ApiServiceError(
                "FORBIDDEN", "cross-tenant audit access is not permitted", 403
            )
        effective_tenant_id = target_tenant_id or tenant.tenant_id
        return [
            _asdict(record)
            for record in store.list(limit=limit, tenant_id=effective_tenant_id)
        ]

    def verify_audit_chain(
        self, *, tenant: "TenantContext | None" = None
    ) -> dict[str, object]:
        """Verify only the current tenant's audit chain for HTTP callers."""
        store = getattr(self, "audit_store", None)
        if store is None:
            return {"valid": True, "checked": 0, "broken_at": None, "head": 0}
        if tenant is None:
            return store.verify_chain()
        return store.verify_chain(tenant_id=tenant.tenant_id)

    def ingest_alert(self, payload: object) -> dict[str, object]:
        ...
        try:
            alerts = AlertWebhookParser().parse_all(
                payload, allow_simplified=diagnoser.client.mode == "mock"
            )
        except ValueError as exc:
            raise ApiServiceError("ALERT_PAYLOAD_INVALID", str(exc)) from exc

        records = [self._ingest_normalized_alert(alert, diagnoser) for alert in alerts]
        if len(records) == 1:
    ...
    def _ingest_normalized_alert(
        self, alert: AlertWebhookPayload, diagnoser: K8sReadOnlyDiagnoser
    ) -> dict[str, object]:
        """Process one normalized alert; durable receipts are added in Phase 4."""
        ...
        # This legacy workflow still owns demo-only diagnostic components. It must
        # not write its conclusions into the shared knowledge base.
        workflow = FaultDiagnoseWorkflow().run(alert.alert_name)
        workflow_payload = {
        ...
        record["audit_status"] = (
            "recorded" if received_audited and processed_audited else "degraded"
        )
        return record
    ...
    def _select_alert_playbook(alert_name: str, labels: dict[str, object]) -> str:
        ...
        text = " ".join(
            [alert_name, *(str(value) for value in labels.values())]
        ).lower()
    ...
    async def run_benchmark(
        self, case_set: str = "default", *, tenant: "TenantContext"
    ) -> BenchmarkRunResponse:
        """
        ...
        self.benchmark_store.save(tenant, response)
        self._audit(tenant.tenant_id, "benchmark.run", run_id, True, case_set)
        return response

    def get_benchmark_report(
        self, run_id: str, *, tenant: "TenantContext"
    ) -> BenchmarkReportResponse:
        """
        ...
        """
        report = self.benchmark_store.get(tenant, run_id)
        if report is None:
            raise ApiServiceError(
                "BENCHMARK_NOT_FOUND",
    ...
    async def run_cloud_ops(
        ...
        started_at = time.perf_counter()
        record = TaskRecord(task_id=task_id, status="running", tenant_id=actor)
        ...
        try:
            ...
            record.status = (
                ...
                else "success" if operation_success else "failed"
    ...
    def _parse_k8s_namespace(task: str, allowlist: tuple[str, ...]) -> str:
        ...
        if allowlist and parsed not in allowlist:
            raise OpsError(
                ErrorCode.ENV_SCOPE_DENIED,
                "namespace is outside the configured allowlist scope",
            )
        return parsed
    ...
    def _run_k8s_ops(
        ...
        action_executor = K8sWriteActionExecutor(
            ...
            K8sActionSecurityPolicy.from_settings(
                self._ops_security or self._load_ops_security()
            ),
            actor=actor,
        )
    ...
    def _k8s_cloud_status(
        ...
        configured_backend = "live" if client.mode == "real" else "mock"
        active_backend = client.last_data_origin
        if active_backend == configured_backend:
            status = "healthy"
        elif active_backend in {"unknown", "unavailable"}:
            status = "unavailable"
        else:
            status = "degraded"
        return {
            "mode": client.mode,
            "source": client.last_data_origin,
            "data_origin": client.last_data_origin,
            "configured_backend": configured_backend,
            "active_backend": active_backend,
            "status": status,
            "reason_code": client.last_error_code,
            "fallback_policy": client.fallback_policy,
            ...
            "prometheus": {
                ...
                "configured_backend": ("live" if prometheus.enabled else "disabled"),
                "active_backend": prometheus.last_data_origin,
                "reason_code": prometheus.last_error_code,
            },
        }

    @staticmethod
    def _load_ops_security() -> "OpsSecuritySettings":
        """Compatibility fallback for direct AthenaWebService construction."""
        from athena.config import load_settings

        return load_settings().ops.security
    ...
    def cleanup_expired_sessions(
        ...
        tenant_id = tenant_id_for(tenant)
        alive_ids = {
            stored.session_id for stored in self.session_store.list(tenant=tenant)
        }
        stale = [
            runtime_key
            for runtime_key in self._agents
            if runtime_key[0] == tenant_id and runtime_key[1] not in alive_ids
        ]
        for runtime_key in stale:
            # 先收集 id 再删除，避免遍历 dict 时修改 dict。
            del self._agents[runtime_key]
            self._agent_config_signatures.pop(runtime_key, None)

    @staticmethod
    def _runtime_key(
        tenant: "TenantContext | None", session_id: str
    ) -> tuple[str, str]:
        return (tenant_id_for(tenant), session_id)

    def _runtime_session(
        self, stored: "StoredSession", *, tenant: "TenantContext | None" = None
    ) -> WebSession:
        """由持久化记录构建运行态 WebSession（含惰性重建的 Agent）。"""
        agent = self._agents.get(self._runtime_key(tenant, stored.session_id))
        return WebSession(
    ...
    def _persist_session(
        self, session: WebSession, *, tenant: "TenantContext | None" = None
    ) -> None:
        """把运行态会话的最新状态写回持久化存储。"""
    ...
    def _require_session(
        ...
        stored = self.session_store.get(session_id, tenant=tenant)
        if stored is None:
            raise ApiServiceError(
                "SESSION_NOT_FOUND",
    ...
    def _ensure_agent(
        ...
        *,
        ...
        """Build or reuse the session Agent, honoring per-request LLM config."""
        signature = self._llm_signature(llm_config)
        runtime_key = self._runtime_key(tenant, session.session_id)
        cached = self._agents.get(runtime_key)
        if (
            cached is not None
            and self._agent_config_signatures.get(runtime_key) == signature
        ):
            session.agent = cached
            return cached
        ...
        if llm_config is None:
            ...
            agent = build_agent(
                ...
                llm_model=self._normalize_model(llm_config.model),
                llm_api_key=(llm_config.api_key or "").strip() or None,
            )
        self._agents[runtime_key] = agent
        if isinstance(agent, ReActAgent):
            agent.trace_recorder = self.decision_trace
        self._agent_config_signatures[runtime_key] = signature
        session.agent = agent
        return agent

    def _project_legacy_trace(
        self, agent: ReActAgent, task_id: str, *, tenant_id: str
    ) -> None:
        """Bind one completed legacy run to a task without exposing hidden reasoning."""
        run_id = getattr(agent, "last_trace_run_id", None)
        if not isinstance(run_id, str) or not run_id:
            return
        self._task_decision_traces[task_id] = self.decision_trace.project_task(
            run_id, task_id=task_id, tenant_id=tenant_id
        )

    def get_decision_trace(self, task_id: str) -> tuple[TaskTraceEvent, ...]:
        """Return structured task telemetry for internal debugging and benchmarks."""
        return self._task_decision_traces.get(task_id, ())

    def resolve_llm_config(
        self,
        tenant: "TenantContext",
        config_id: str | None,
        legacy: LLMConfig | None,
    ) -> LLMConfig | None:
        """Resolve a server-managed model selection while retaining API compatibility."""
        if not config_id:
            if legacy is not None and not self._allow_legacy_llm_credentials:
                raise ApiServiceError(
                    "LLM_LEGACY_CREDENTIAL_FORBIDDEN",
                    "production requires a managed LLM configuration",
                    status_code=400,
                )
            return legacy
        stored = self.llm_config_store.get(tenant, config_id)
        if stored is None or not stored.enabled or stored.status != "available":
            raise ApiServiceError(
                "LLM_CONFIG_UNAVAILABLE", "所选模型配置不可用", status_code=400
            )
        from athena.infra.secret_store import SecretStoreError

        try:
            api_key = self.llm_config_store.resolve_api_key(tenant, stored)
        except SecretStoreError as exc:
            raise ApiServiceError(
                "LLM_CREDENTIAL_UNAVAILABLE", "模型凭证不可用", status_code=400
            ) from exc
        return LLMConfig(provider="litellm", model=stored.model, api_key=api_key)
    ...
    def _llm_signature(
        ...
        return (
            llm_config.provider,
            self._normalize_model(llm_config.model),
    ...
    def _record_failure(
        ...
        *,
        ...
        record.updated_at = time.time()
        self.task_store.save(record, tenant=tenant)  # 失败状态也落库，保证可查
