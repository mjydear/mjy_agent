...
_VALID_FALLBACK_POLICIES = ("allow_mock", "fail_closed")
...
class K8sReadOnlyClient:
    ...
    def __init__(
        ...
        *,
        ...
        normalized_fallback_policy = (fallback_policy or "").strip().lower()
        if normalized_fallback_policy not in _VALID_FALLBACK_POLICIES:
            raise OpsError(
                ErrorCode.CONFIG_INVALID,
                "ops.kubernetes.fallback_policy must be one of "
                f"{_VALID_FALLBACK_POLICIES}, got: {fallback_policy!r}",
            )
        self.fallback_policy = normalized_fallback_policy
        self.last_data_origin = "mock" if self.mode == "mock" else "unknown"
    ...
    def from_settings(cls, settings: AthenaSettings) -> K8sReadOnlyClient:
        ...
        return cls(
            ...
            fallback_policy=ops.kubernetes.fallback_policy,
        )
    ...
    def _run_with_fallback(
        ...
        if self.mode != "real":
            ...
            return self._with_data_origin(mock_fn(), "mock")
        try:
            self.last_data_origin = "live"
            self.last_error = None
            self.last_error_code = None
            return self._with_data_origin(real_fn(), "live")
            ...
            self.last_error = error_code.value
            if self.fallback_policy == "fail_closed":
                self.last_data_origin = "unavailable"
                raise OpsError(
                    error_code,
                    "live Kubernetes data is unavailable",
                ) from exc
            logger.warning(
                "k8s real call failed, falling back to mock: %s",
                error_code.value,
            )
            self.last_data_origin = "mock"
            return self._with_data_origin(mock_fn(), "mock")
    ...
    def _classify_live_error(exc: Exception) -> ErrorCode:
        """Map SDK/network failures to stable codes without exposing details."""
        ...
        return ErrorCode.ENV_CONNECTION_FAILED

    @staticmethod
    def _with_data_origin(result: _T, data_origin: str) -> _T:
        """Annotate structured K8s observations with their actual origin."""
        if isinstance(result, dict):
            return cast(_T, {**result, "data_origin": data_origin})
    ...
    def list_services(self, namespace: str = "default") -> list[dict[str, JSONValue]]:
        """
    ...
    def list_endpoints(self, namespace: str = "default") -> list[dict[str, JSONValue]]:
        """
    ...
    def _real_describe_pod(self, namespace: str, name: str) -> dict[str, JSONValue]:
        api = self._get_core_api()
        ...
        spec_ports: dict[str, list[int]] = {
            container.name: [
                p.container_port for p in (getattr(container, "ports", None) or [])
            ]
        ...
        return {
            ...
            "containers": [
                {
                    **container,
                    "ports": spec_ports.get(str(container.get("name", "")), []),
                }
    ...
    def _real_list_deployments(self, namespace: str) -> list[dict[str, JSONValue]]:
            ...
            ports: list[dict[str, JSONValue]] = [
                {
                    "port": p.port,
                    "target_port": (
                        str(p.target_port) if p.target_port is not None else ""
                    ),
    ...
    def _real_get_node_status(self) -> list[dict[str, JSONValue]]:
        ...
        for node in nodes:
            ...
            pressure = [
                c.type for c in conditions if c.type != "Ready" and c.status == "True"
            ]
    ...
    def _mock_describe_pod(self, namespace: str, name: str) -> dict[str, JSONValue]:
        return {
            "name": name,
            "namespace": namespace,
            "status": "CrashLoopBackOff",
    ...
    def _mock_get_pod_logs(
        ...
        lines = [
            f"[mock][{namespace}/{name}] starting container " f"{container or 'main'}",
    ...
    def _mock_list_deployments(self, namespace: str) -> list[dict[str, JSONValue]]:
        return [
            {
                "name": "api",
                "namespace": namespace,
    ...
    def _mock_list_services(self, namespace: str) -> list[dict[str, JSONValue]]:
        return [
            ...
            },
            {
                ...
                "ports": [{"port": 80, "target_port": "9090", "protocol": "TCP"}],
            },
        ]
