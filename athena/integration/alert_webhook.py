...
class AlertWebhookParser:
    ...
    def parse_all(
        self, payload: object, *, allow_simplified: bool = False
    ) -> tuple[AlertWebhookPayload, ...]:
        """Normalize every Alertmanager alert without retaining sensitive metadata."""
        if not isinstance(payload, Mapping):
            raise ValueError("Alertmanager payload must be a JSON object")
        raw_alerts = payload.get("alerts")
        if raw_alerts is None:
            if not allow_simplified or not str(payload.get("alert_name", "")).strip():
                raise ValueError("Alertmanager payload requires a non-empty alerts list")
            raw_alerts = [payload]
        if not isinstance(raw_alerts, list) or not raw_alerts:
            raise ValueError("Alertmanager alerts must be a non-empty list")
        normalized: list[AlertWebhookPayload] = []
        for item in raw_alerts:
            if not isinstance(item, Mapping):
                raise ValueError("Alertmanager alerts must contain JSON objects")
            normalized.append(self._normalize_alert(item, payload))
        return tuple(normalized)

    def _normalize_alert(
        self, alert: Mapping[str, object], fallback: Mapping[str, object]
    ) -> AlertWebhookPayload:
        raw_labels = alert.get("labels", {})
        raw_annotations = alert.get("annotations", {})
        if not isinstance(raw_labels, Mapping) or not isinstance(raw_annotations, Mapping):
            raise ValueError("Alertmanager labels and annotations must be JSON objects")
        labels = self._redact_metadata(raw_labels)
        annotations = self._redact_metadata(raw_annotations)
        alert_name = str(labels.get("alertname", fallback.get("alert_name", ""))).strip()
        if not alert_name:
            raise ValueError("Alertmanager alertname must be non-empty")
        return AlertWebhookPayload(
            alert_name=alert_name,
            severity=str(labels.get("severity", "warning")),
            namespace=str(labels.get("namespace", fallback.get("namespace", "default"))),
            pod=str(labels.get("pod", fallback.get("pod", ""))),
            deployment=str(labels.get("deployment", fallback.get("deployment", ""))),
            summary=str(annotations.get("summary", fallback.get("summary", "")))[:2000],
            description=str(annotations.get("description", fallback.get("description", "")))[:8000],
            labels=labels,
            annotations=annotations,
        )

    def _redact_metadata(self, metadata: Mapping[str, object]) -> dict[str, JSONValue]:
        ...
        for key, value in metadata.items():
            ...
            if any(
                ...
                result[normalized_key] = "[UNSUPPORTED_METADATA]"
        return result
