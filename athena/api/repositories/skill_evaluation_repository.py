...
from .models import (
    SkillBaselineRunModel,
    SkillReplayABRunModel,
    SkillShadowRunModel,
)
...
class SkillEvaluationRepository:
            ...
            async with self._sessions() as session:
                async with session.begin():
                    ...
                    if run.gate_passed:
                        ...
                        candidate.replay_report_id = run.run_id
    ...
    async def find_shadow(
        ...
        async with self._sessions() as session:
            model = await session.scalar(
                select(SkillShadowRunModel).where(
                    SkillShadowRunModel.tenant_id == tenant_id,
                    SkillShadowRunModel.candidate_id == candidate_id,
                    SkillShadowRunModel.candidate_digest == candidate_digest,
                    SkillShadowRunModel.case_definition_digest
                    == case_definition_digest,
                    SkillShadowRunModel.runner == runner,
                )
            )
            return None if model is None else self._shadow_from_model(model)

    async def get_shadow(self, tenant_id: str, run_id: str) -> ShadowRun | None:
        async with self._sessions() as session:
            model = await session.scalar(
                select(SkillShadowRunModel).where(
                    SkillShadowRunModel.tenant_id == tenant_id,
                    SkillShadowRunModel.id == run_id,
                )
            )
            return None if model is None else self._shadow_from_model(model)

    async def save_shadow(self, run: ShadowRun) -> ShadowRun:
        """Persist Shadow observations without changing Candidate lifecycle state."""
        ...
        try:
            async with self._sessions() as session:
                async with session.begin():
                    existing = await session.scalar(
                        select(SkillShadowRunModel).where(
                            SkillShadowRunModel.tenant_id == run.tenant_id,
                            SkillShadowRunModel.id == run.run_id,
                        )
                    )
                    if existing is not None:
                        return self._shadow_from_model(existing)
                    candidate = await session.scalar(
                        select(SkillCandidateModel)
                        .where(
                            SkillCandidateModel.tenant_id == run.tenant_id,
                    ...
                    if (
                        ...
                        raise SkillCandidateLifecycleError(
                            "SKILL_CANDIDATE_SHADOW_STATE_INVALID"
                        )
                    candidate.shadow_report_id = run.run_id
                    candidate.audit_events_json = [
                        *(candidate.audit_events_json or []),
                        {
                            "kind": "candidate.shadow_evaluated",
                            "at": run.completed_at.isoformat(),
                            "run_id": run.run_id,
                            "gate_passed": run.gate_passed,
                            "activation_allowed": False,
                    ...
                    session.add(model)
                    await session.flush()
                    return self._shadow_from_model(model)
        except IntegrityError:
            existing = await self.get_shadow(run.tenant_id, run.run_id)
            if existing is None:
                raise
            return existing
    ...
    @staticmethod
    def _shadow_from_model(model: SkillShadowRunModel) -> ShadowRun:
        comparisons = tuple(
            ShadowCaseComparison(
                case_id=str(item.get("case_id") or ""),
                category=str(item.get("category") or ""),
                main=_shadow_metrics(item.get("main"), "main"),
                shadow=_shadow_metrics(item.get("shadow"), "shadow"),
                main_result_preserved=bool(item.get("main_result_preserved")),
                result_consistent=bool(item.get("result_consistent")),
                shadow_side_effects_detected=bool(
                    item.get("shadow_side_effects_detected")
                ),
                deltas={
                    str(key): float(value)
                    for key, value in dict(item.get("deltas") or {}).items()
...
def _shadow_metrics(value: object, group: str) -> ShadowRuntimeMetrics:
    raw = dict(value) if isinstance(value, dict) else {}
    return ShadowRuntimeMetrics(
        group="main" if group == "main" else "shadow",
        task_status=str(raw.get("task_status") or "failed"),
        task_success=bool(raw.get("task_success")),
        candidate_loaded=bool(raw.get("candidate_loaded")),
        candidate_read_count=int(raw.get("candidate_read_count") or 0),
        trigger_matched=bool(raw.get("trigger_matched")),
        loaded_layers=tuple(str(item) for item in raw.get("loaded_layers", [])),
        evidence_retention=float(raw.get("evidence_retention") or 0.0),
        tick_count=int(raw.get("tick_count") or 0),
        tool_call_count=int(raw.get("tool_call_count") or 0),
        input_tokens=int(raw.get("input_tokens") or 0),
        ...
        effect_count=int(raw.get("effect_count") or 0),
        side_effects_detected=bool(raw.get("side_effects_detected")),
        successful_tool_calls=tuple(
            str(item) for item in raw.get("successful_tool_calls", [])
        ),
        rejected_tool_calls=tuple(
