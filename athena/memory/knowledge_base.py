...
class KnowledgeBaseManager:
    ...
    def __init__(
        ...
        # VectorStore does not yet expose a metadata-filter contract. Keep the
        # count so recall can inspect every locally indexed candidate before
        # applying the tenant boundary below.
    ...
    async def ingest(
        ...
        *,
        ...
        self._docs.setdefault(tenant_id, {})[doc_id] = doc
        self._indexed_chunk_count += len(chunks)
        return doc

    def list_documents(self, *, tenant_id: str = "public") -> list[KnowledgeDocument]:
        """列出所有已入库文档（按创建时间倒序）。"""
        tenant_id = self._require_tenant_id(tenant_id)
        return sorted(
            self._docs.get(tenant_id, {}).values(),
            key=lambda d: d.created_at,
            reverse=True,
        )

    def get_document(
        self, doc_id: str, *, tenant_id: str = "public"
    ) -> KnowledgeDocument | None:
        tenant_id = self._require_tenant_id(tenant_id)
        return self._docs.get(tenant_id, {}).get(doc_id)

    def delete_document(self, doc_id: str, *, tenant_id: str = "public") -> bool:
    ...
    async def recall(
        ...
        *,
        ...
        candidates = await self._store.search(
            embedding,
            top_k=max(rule.top_k * 4, rule.top_k, self._indexed_chunk_count),
        )
        tenant_docs = self._docs.get(tenant_id, {})
        results: list[RecallResult] = []
        for doc in candidates:
            # Missing metadata is treated as legacy public content. It is never
            # exposed to a non-public tenant after the multi-tenant migration.
            indexed_tenant = doc.metadata.get("tenant_id", "public")
            if indexed_tenant != tenant_id:
                continue
            knowledge_doc = tenant_docs.get(str(doc.metadata.get("kb_doc_id", "")))
            # Deleted documents can remain in a vector backend until compaction;
            # the tenant's metadata directory is the authorization check.
