...
class LLMClientFactory:
    ...
    def create(
        ...
        if required_env and not (api_key or os.getenv(required_env)):
...
def _parse_usage_normalized(raw_usage: object) -> dict[str, int]:
    """Normalize provider usage without serializing arbitrary response data."""
    usage_map = _usage_mapping(raw_usage)
    if not usage_map:
        return {}
    ...
    for key, value in usage_map.items():
        name = str(key)
        numeric_value = _usage_number(value)
        if numeric_value is not None:
            parsed[name] = numeric_value
            continue
        if _is_usage_details(name):
            _flatten_usage_details(value, nested)

    # Top-level values are authoritative when a provider exposes both a
    # canonical field and a details field with the same leaf name.
    ...
    if cached_tokens is None and cache_read_input_tokens is not None:
        parsed["cached_tokens"] = cache_read_input_tokens
    elif cache_read_input_tokens is None and cached_tokens is not None:
        parsed["cache_read_input_tokens"] = cached_tokens

    return parsed


def _usage_mapping(raw_usage: object) -> Mapping[object, object]:
    """Return usage fields without serializing arbitrary provider objects."""
    ...
    # Include slot-backed/provider objects that do not expose __dict__.
    known_fields = (
        "prompt_tokens",
        "input_tokens",
        "completion_tokens",
...
def _usage_number(value: object) -> int | None:
    """Convert integral token counters while dropping non-numeric metadata."""
...
def _is_usage_details(name: str) -> bool:
    return name.endswith("_details") or name in {
        "prompt_tokens_details",
        "input_tokens_details",
        "completion_tokens_details",
        "output_tokens_details",
...
def _flatten_usage_details(
    ...
    """Promote numeric leaves from a provider's nested usage details."""
    ...
    details = _usage_mapping(value)
    for key, nested_value in details.items():
        name = str(key)
        numeric_value = _usage_number(nested_value)
        if numeric_value is not None:
            target.setdefault(name, numeric_value)
            continue
        _flatten_usage_details(nested_value, target, seen)
...
def _parse_usage(raw_usage: object) -> dict[str, int]:
    ...
    代价是代码稍复杂，但换来了更强的稳定性。
    """
    return _parse_usage_normalized(raw_usage)
