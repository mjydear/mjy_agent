...
const elements = {
  ...
  apiKeyTrigger: document.getElementById("api-key-trigger"),
  apiKeyState: document.getElementById("api-key-state"),
  apiKeyModal: document.getElementById("api-key-modal"),
  apiKeyForm: document.getElementById("api-key-form"),
  apiKeyClose: document.getElementById("api-key-close"),
  ...
  llmBaseUrlField: document.getElementById("llm-base-url-field"),
  llmConfigList: document.getElementById("llm-config-list"),
  llmFormError: document.getElementById("llm-form-error"),
  opsTaskMode: document.getElementById("ops-task-mode"),
  opsTaskWorkbench: document.getElementById("ops-task-workbench"),
  consoleComposer: document.getElementById("console-composer"),
};
...
function getLlmConfigPayload() {
  return state.llmConfigId ? { llm_config_id: state.llmConfigId } : {};
}

function renderLlmSettings() {
  const available = state.llmConfigs.filter((item) => item.enabled && item.status === "available");
  let selected = available.find((item) => item.config_id === state.llmConfigId);
  if (!selected) selected = available.find((item) => item.is_default) || available[0];
  state.llmConfigId = selected?.config_id || null;
  elements.modelCurrent.textContent = selected?.display_name || "配置模型";
  elements.modelDot.classList.toggle("unconfigured", !selected);
  elements.apiKeyState.textContent = available.length ? `${available.length} 个可用` : "未配置";
  elements.apiKeyTrigger?.classList.toggle("configured", Boolean(available.length));
  elements.modelMenu.innerHTML = available.map((item) => `<button class="model-option${item.config_id === state.llmConfigId ? " active" : ""}" data-config-id="${escapeHtml(item.config_id)}" role="option"><strong>${escapeHtml(item.display_name)}</strong><small>${escapeHtml(item.provider)} · ${escapeHtml(item.model)}</small></button>`).join("");
  elements.llmConfigList.innerHTML = state.llmConfigs.length ? state.llmConfigs.map((item) => `<div class="llm-config-row"><div><strong>${escapeHtml(item.display_name)}</strong><small>${escapeHtml(item.provider)} · ${escapeHtml(item.model)}</small></div><button type="button" class="icon-btn llm-delete" data-config-id="${escapeHtml(item.config_id)}" title="删除模型配置">×</button></div>`).join("") : `<div class="llm-empty">尚未配置模型。添加后才会出现在顶部模型列表中。</div>`;
}

async function loadLlmConfigs() {
  state.llmConfigs = await api("/api/llm/configs");
  renderLlmSettings();
}
...
async function sendMessage() {
  if (state.mode === "ops-task") return;
  if (state.mode === "chat" && !state.llmConfigId) {
    openLlmSettings();
    if (elements.llmFormError) elements.llmFormError.textContent = "请先添加一个可用模型，再开始对话。";
    return;
  }
...
function setMode(mode) {
  state.mode = mode;
  const titles = { chat: "单 Agent 对话", workflow: "多 Agent 工作流", benchmark: "Benchmark 评测", cloud: "云运维模式", "ops-task": "故障任务工作台" };
  ...
  const opsTaskActive = mode === "ops-task";
  elements.chatLog.hidden = opsTaskActive;
  if (elements.opsTaskWorkbench) elements.opsTaskWorkbench.hidden = !opsTaskActive;
  if (elements.consoleComposer) elements.consoleComposer.hidden = opsTaskActive;
  if (elements.detailAside) {
    elements.detailAside.hidden = opsTaskActive;
    if (opsTaskActive) elements.detailPanel.textContent = "";
    else renderDetailPanel();
  }
  window.AthenaOpsTaskWorkbench?.setActive(opsTaskActive);
 ...
 */
function setModel(configId) {
  state.llmConfigId = configId;
  renderLlmSettings();
...
elements.modelTrigger?.addEventListener("click", (event) => {
  event.stopPropagation();
  if (!state.llmConfigs.some((item) => item.enabled && item.status === "available")) {
    openLlmSettings();
    return;
  }
...
elements.modelMenu?.addEventListener("click", (event) => {
  ...
  if (option) setModel(option.dataset.configId);
});

elements.apiKeyTrigger?.addEventListener("click", openLlmSettings);
elements.apiKeyClose?.addEventListener("click", closeLlmSettings);
elements.llmProvider?.addEventListener("change", () => {
  const needsUrl = ["ollama", "openai_compatible"].includes(elements.llmProvider.value);
  elements.llmBaseUrlField.hidden = !needsUrl;
  elements.llmApiKey.closest("label").hidden = elements.llmProvider.value === "ollama";
});
elements.llmConfigList?.addEventListener("click", async (event) => {
  const button = event.target.closest(".llm-delete");
  if (!button) return;
  await api(`/api/llm/configs/${button.dataset.configId}`, { method: "DELETE" });
  await loadLlmConfigs();
});
elements.apiKeyForm?.addEventListener("submit", async (event) => {
  event.preventDefault();
  elements.llmFormError.textContent = "";
  try {
    await api("/api/llm/configs", { method: "POST", body: JSON.stringify({
      provider: elements.llmProvider.value, display_name: elements.llmDisplayName.value,
      model: elements.llmModel.value, api_key: elements.llmApiKey.value || null,
      base_url: elements.llmBaseUrl.value || null,
    }) });
    elements.apiKeyForm.reset();
    await loadLlmConfigs();
  } catch (error) {
    elements.llmFormError.textContent = error.message;
  }
});
elements.apiKeyModal?.addEventListener("click", (event) => {
  if (event.target === elements.apiKeyModal) closeLlmSettings();
});
...
async function init() {
  await loadLlmConfigs();
  await loadSessions();
  const opsTaskAvailable = await window.AthenaOpsTaskWorkbench?.init();
  if (elements.opsTaskMode) elements.opsTaskMode.hidden = !opsTaskAvailable;
