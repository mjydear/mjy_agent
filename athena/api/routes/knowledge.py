...
async def list_documents(
    ...
    docs = [
        ...
        for d in kb.list_documents(tenant_id=tenant.tenant_id)
    ]
...
async def delete_document(
    ...
    """删除一篇知识文档。"""
    if not kb.delete_document(doc_id, tenant_id=tenant.tenant_id):
        raise ApiServiceError(
            "KNOWLEDGE_NOT_FOUND", "document not found", status_code=404
        )
...
async def recall(
    ...
    try:
        hits = await kb.recall(payload.query, rule, tenant_id=tenant.tenant_id)
    except ValueError as exc:
        raise ApiServiceError("KNOWLEDGE_INVALID", str(exc), status_code=400) from exc
    return ApiResponse.ok(
        [
