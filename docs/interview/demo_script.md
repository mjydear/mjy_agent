...
大家好，我演示的是 Athena Agent，一个面向 Kubernetes 云运维的自进化 Agent。它的重点不是套一个聊天框，而是把告警接入、只读诊断、Evidence、人工反馈和 Skill 治理做成完整闭环。今天我会演示四个场景：自进化、K8s 排障、告警处置和 Multi-Agent 协作。
...
现在运行 `python examples/demo_multi_agent.py`。规划、执行和校验角色共享受控的任务上下文，但工具权限和最终输出仍由统一 Runtime 管理，避免子 Agent 越权调用。
