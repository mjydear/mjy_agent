...
class ToolRegistry:
    ...
    def get_tool(self, name: str) -> Tool | None:
        """Return registered metadata without invoking the legacy tool."""
    ...
    async def invoke(self, call: ToolCall) -> ToolResult:
        ...
        try:
            # Start blocking legacy adapters in a worker thread so an outer
            # asyncio timeout can fire while the adapter is still running.
            result: object
            if inspect.iscoroutinefunction(tool.handler):
                result = tool.handler(**call.arguments)
            else:
                sync_handler = cast(Callable[..., object], tool.handler)
                result = await asyncio.to_thread(sync_handler, **call.arguments)
