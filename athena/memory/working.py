...
💡 设计思路：
   ...
   Token 计数通过可替换的 TokenMeter 完成；没有模型 tokenizer 时，
   回退策略对 CJK 采用保守计数，避免上下文预算被低估。
📚 学习重点：
...
class WorkingMemory(BaseModel):
    ...
    字段说明：
        max_tokens: 记忆的最大 Token 容量（默认 8000）。
                    ...
                    计数由 TokenMeter 提供；可注入模型 tokenizer，缺省时采用保守回退。
    ...
    token_meter: TokenMeter = Field(default_factory=TokenMeter, exclude=True)
    ...
    def render_for_budget(self, max_tokens: int) -> str:
        """Render a non-mutating session-memory projection within ``max_tokens``."""

        if isinstance(max_tokens, bool) or not isinstance(max_tokens, int):
            raise ValueError("max_tokens must be a non-negative integer")
        if max_tokens < 0:
            raise ValueError("max_tokens must be a non-negative integer")
        if max_tokens == 0 or not self.messages:
            return ""
        ...
        candidates = sorted(
            ...
            key=lambda index: (
                index == last_index,
                self.messages[index].importance,
                index,
            ),
            reverse=True,
        )
        ...
        for index in candidates:
            ...
            candidate = {**selected, index: message.content}
            if self._render_entries(candidate) <= max_tokens:
                selected[index] = message.content
                continue
            if index == last_index and not selected:
                prefix = f"{message.role}: "
                compact = self._truncate_text_to_budget(
                    message.content,
                    max_tokens=max_tokens,
                    prefix=prefix,
                )
                if compact or self.token_meter.count(prefix) <= max_tokens:
                    selected[index] = compact

        return self._render_entries(selected, count_only=False)
    ...
    def _prune_if_needed(self) -> None:
        ...
        if len(self.messages) == 1 and self._estimated_tokens() > self.max_tokens:
            message = self.messages[0]
            compact = (
                message.content
                if message.compressed
                else self._summarize_messages([message])
            )
            self.messages[0] = Message(
                role=message.role,
                content=self._truncate_text_to_budget(
                    compact,
            ...
            logger.debug("Truncated oversized working-memory message role=%s", message.role)
    ...
    def _estimated_tokens(self) -> int:
        """Return current message usage through the shared TokenMeter."""
    ...
    def _estimate_text_tokens(self, text: str) -> int:
        return max(1, self.token_meter.count(text))

    def _render_entries(
        self, entries: dict[int, str], *, count_only: bool = True
    ) -> int | str:
        rendered = "\n".join(
            f"{self.messages[index].role}: {content}"
            for index, content in sorted(entries.items())
        )
        if count_only:
            return self.token_meter.count(rendered)
        return rendered
    ...
    def _truncate_text_to_budget(
        ...
        *,
        ...
        if max_tokens <= 0 or self.token_meter.count(prefix) > max_tokens:
            return ""
        if self.token_meter.count(prefix + text) <= max_tokens:
            return text
        ...
        while low < high:
            middle = (low + high + 1) // 2
            if self.token_meter.count(prefix + text[:middle]) <= max_tokens:
                low = middle
            else:
                high = middle - 1
        ...
        compact = text[:low]
        marker = "..."
        if low < len(text) and self.token_meter.count(prefix + compact + marker) <= max_tokens:
